...
public class LanceItem extends Item implements Vanishable {
	...
	public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
		if (!world.isClient && user instanceof PlayerEntity player) {
			...
			if (level >= 10) {
				if (player instanceof ChargeableEntity charger) {
					charger.charge(40);
					charger.chargeVelocity(player, 4.0F);
				}
	...
	public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
		if (!world.isClient && entity instanceof ServerPlayerEntity player && player.getVehicle() instanceof HorseEntity horse && stack.isOf(ModItems.LANCE) && selected == true) {
			...
			if (level >= 1) {
				player.addStatusEffect(new StatusEffectInstance(ModStatusEffects.CHARGE, 10, 1, true, true));
